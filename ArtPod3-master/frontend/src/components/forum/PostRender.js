...
function PostRender(props) {
  ...
  const renderComments = () => {
    ...
    return props.commentsList.map((comment)=> {
      const handleDeleteComment = async () => {
        const data = await ArtPodAPI.deleteItem(item, comment.id)
        if (data) {
      ...
      return <div>
        <div className="post-cont mt-3">
          ...
          <Row className="detail-cont">
            <Col>
              <img className="profile-pic" src="default_pic.png" alt = "No Photo"/>
  ...
  const handleDeletePost = async () => {
    const item = "posts"
    const data = await ArtPodAPI.deleteItem(item, props.id)
    if (data) {
