...
function PostDetailsPage(props) {
  ...
  const loadPost = async () => {
    const item = "posts"
    const data = await ArtPodAPI.getItemById(item, postId)
  ...
  const loadComments = async () => {
    const comments = []
    const data = await ArtPodAPI.getAllItems(itemComment)
  ...
  const handleCreateComment = async (event) => {
    ...
    const data = await ArtPodAPI.createItems(itemComment, commentData)
